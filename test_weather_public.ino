...
/*
 * Made some correction to make it work on Arduino 1.8.19
 * L.Labbe January 2023
 */
...
#include "owm_credentials_o.h"   // Replace your own credential for openweather. Also, this is for Hong Kong time
...
#define Small  9            // 6  For icon drawing, needs to be odd number for best effect
...
boolean SetupTime() {
  ...
  setenv("TZ", Timezone, 0);  //setenv()adds the "TZ" variable to the environment with a value TimeZone, only used if set to 1, 0 means no change
...
void loop() {
  ...
  Temp_int_str = epd_ambient_temperature();
  Delay(300);
...
bool getRandomImage(WiFiClient & client) {
  ...
  {
    Serial.printf("get image connection failed, error: %s\n\r", http.errorToString(httpCode).c_str());
    client.stop();
    http.end();
    return false;
  }
...
bool obtainWeatherData(WiFiClient & client, const String & RequestType) {
  ...
  {
    Serial.printf("Get Weather connection failed, error: %s\n\r", http.errorToString(httpCode).c_str());
    client.stop();
    http.end();
    return false;
  }
...
//    DisplayAstronomySection(920, 720);             // Astronomy section Sun rise/set, Moon phase and Moon icon
    DisplayAstronomySection(880, 400);             // Astronomy section Sun rise/set, Moon phase and Moon icon
...
void DisplayGeneralInfoSection() {
  ...
  drawString(600, 2, TXT_INT_TEMP + Temp_int_str, LEFT);
}

void DisplayMainWeatherSection(int x, int y) {  // (x=500, y=190)
  Serial.println("Display conditions");
  ...
  Serial.println("Display rainfall");
...
void DisplayForecastSection(int x, int y) {
  ...
  DrawGraph(gx + 3 * gap + 5, gy, gwidth, gheight, 900, 1050, Units == "M" ? TXT_PRESSURE_HPA : TXT_PRESSURE_IN, pressure_readings, max_readings, autoscale_on, barchart_off);
  ...
  DrawGraph(gx + 2 * gap + 4, gy, gwidth, gheight, 0, 100,   TXT_HUMIDITY_PERCENT, humidity_readings, max_readings, autoscale_off, barchart_off);
...
//  float voltage = analogRead(34) / 4096.0 * 7.46;
  // float voltage = esp_adc_cal_raw_to_voltage(34, &adc_chars)* (180+680))/180;  // R1 = 680k / R2 = 180k
  float voltage = ((float)analogRead(34) * (180+680))/180;  // R1 = 680k / R2 = 180k
